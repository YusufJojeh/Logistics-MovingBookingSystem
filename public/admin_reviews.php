...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'add') {
    // DB schema requires booking_id NOT NULL
    ...
    // Ensure booking matches client/provider and is completed
    if (!$errors) {
...
/* Completed bookings for Add modal (with names) */
...
<head>
  ...
  <!-- Page-local fixes: keep dropdowns visible above glass/table scroll -->
  <style>
    .overflow-visible { overflow: visible !important; }
    .z-dropdown .dropdown-menu { z-index: 2000; } /* above cards/tables */
  </style>
</head>
<body class="modern-bg">
        ...
        <div class="row g-4">
          <div class="col-md-3">
            <div class="kpi-card text-center">
              ...
              <div class="kpi-number"><?= (int)$total_reviews ?></div>
              <div class="kpi-label">Total Reviews</div>
              <div class="kpi-trend text-success"><i class="bi bi-arrow-up"></i> All Time</div>
            </div>
          </div>
          <div class="col-md-3">
            <div class="kpi-card text-center">
          ...
          <div class="col-md-3">
            <div class="kpi-card text-center">
              <div class="kpi-icon"><i class="bi bi-star-fill"></i></div>
              <div class="kpi-number"><?= (int)$five_star_reviews ?></div>
              <div class="kpi-label">5-Star Reviews</div>
              <div class="kpi-trend text-success"><i class="bi bi-arrow-up"></i> <?= $five_pct ?>%</div>
            </div>
          </div>
          <div class="col-md-3">
            <div class="kpi-card text-center">
              <div class="kpi-icon"><i class="bi bi-calendar-event"></i></div>
              <div class="kpi-number"><?= (int)$recent_reviews ?></div>
              <div class="kpi-label">Recent Reviews</div>
              <div class="kpi-trend text-info"><i class="bi bi-arrow-up"></i> Last 30 Days</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="glass-card overflow-visible">
          <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="d-flex gap-2">
              <button class="btn btn-outline-primary" type="button" onclick="exportReviews()">
                <i class="bi bi-download me-2"></i>Export
              </button>
              <button class="btn btn-primary" type="button" data-bs-toggle="modal" data-bs-target="#addReviewModal">
            ...
            <div class="table-responsive overflow-visible">
              <table class="table table-dark align-middle">
                ...
                <tbody>
                      ...
                      <tr id="row-<?= (int)$r['id'] ?>">
                        ...
                        <td>
                          <div class="d-flex align-items-center">
                            ...
                            <span class="fw-semibold"><?= e($r['client_name']) ?></span>
                          </div>
                        </td>
                        <td>
                          <div class="d-flex align-items-center">
                            <div class="user-avatar me-2"><i class="bi bi-building"></i></div>
                            <span class="fw-semibold"><?= e($r['provider_name']) ?></span>
                          </div>
                        </td>
                        <td>
                          <div class="d-flex align-items-center">
                            <div class="me-2">
                              <?php for($i = 1; $i <= 5; $i++): ?>
                                <i class="bi bi-star<?= $i <= (int)$r['rating'] ? '-fill' : '' ?> text-warning"></i>
                              <?php endfor; ?>
                            </div>
                          </div>
                        </td>
                        <td>
                          <div class="text-truncate" style="max-width: 320px;" title="<?= e($r['comment']) ?>">
                            <?= e($r['comment']) ?>
                          </div>
                        </td>
                  ...
                  <?php else: ?>
                    <tr><td colspan="7">
                      <div class="table-empty"><i class="bi bi-inbox"></i><div>No reviews found.</div></div>
                    </td></tr>
                  <?php endif; ?>
                </tbody>
              </table>
              ...
              <div class="col-md-6">
                ...
                <select class="form-select" name="booking_id" id="add_booking" required>
                  ...
                  <?php while($b = mysqli_fetch_assoc($bookings_rs)): ?>
                    <option
                      value="<?= (int)$b['id'] ?>"
                      data-client_name="<?= e($b['client_name']) ?>"
                      data-provider_name="<?= e($b['provider_name']) ?>"
                    >
                      #<?= (int)$b['id'] ?> — <?= e($b['client_name']) ?> → <?= e($b['provider_name']) ?> (<?= e($b['booking_date']) ?>)
                    </option>
                  <?php endwhile; ?>
                </select>
                <div class="form-text">Must be a completed booking between the selected client & provider.</div>
              </div>
  ...
  <script>
    // Export placeholder
    function exportReviews(){ if (typeof showToast==='function') showToast('Export feature coming soon!', 'info'); }
    ...
    // Make dropdowns robust against clipping (fixed strategy)
